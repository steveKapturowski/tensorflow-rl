...
class Emulator:
    def __init__(self, rom_path, rom_name, visualize, actor_id):
        ...
        self.ale.setInt("random_seed", 123 * actor_id)
    ...
    def get_screen_image(self):
        ...
        self.frame_pool[self.current] = np.squeeze(screen_image)
        self.current = (self.current + 1) % FRAMES_IN_POOL
        return screen_image_rgb
    ...
    def process_frame_pool(self):
        ...
        down_sampled_img = cv2.resize(cropped_img, (84, 110), 
    ...
    def get_initial_state(self, life_loss=True):
        ...
        if self.is_terminal(life_loss):
            MAX_START_WAIT -= 1
            return self.get_initial_state()
        return self.get_reshaped_state()      

    def next(self, action, life_loss=True):
        ...
        self.show_screen(new_screen_image_rgb)
        terminal = self.is_terminal(life_loss)
        self.lives = self.ale.lives()
        return self.get_reshaped_state(), reward, terminal
    ...
    def is_terminal(self, life_loss):
        if life_loss:
...
if __name__ == "__main__":
    emulator = Emulator("../atari_roms", "breakout", True, 0)
