...
if __name__ == '__main__':
    ...
    parser.add_argument('--opt_mode', default="shared", help="Whether to use \"local\" or \"shared\" vector(s) for the moving average(s)", dest="opt_mode")
    ...
    parser.add_argument('--alpha', default=0.99, type=float, help="Discounting factor for the history/coming gradient, for the Rmsprop optimizer", dest="alpha")
    parser.add_argument('-lr', '--initial_lr', default=get_learning_rate(10**-4, 10**-2), type=float, help="Initial value for the learning rate. Default = LogUniform(10**-4, 10**-2)", dest="initial_lr")
    ...
    parser.add_argument('--grads_update_steps', default=5, type=int, help="Nr. of local steps during which grads are accumulated before applying them to the shared network parameters", dest="grads_update_steps")
    ...
    parser.add_argument('--max_local_steps', default=5, type=int, help="Number of steps for the Q learning/A3C algorithm", dest="max_local_steps")
